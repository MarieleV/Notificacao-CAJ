...
export function OuvidoriaManager() {
  ...
  const handleGenerateParecer = async () => {
    ...
    // Define o texto base do objeto de acordo com a infração selecionada
    let textoObjeto = "";
    switch (tipoCaso) {
    ...
    try {
      const response = await fetch("https://notificacao-caj.vercel.app/api/gerar", {
        ...
        body: JSON.stringify({
          api_key: apiKey,
          textos_base: [
            `AJE/OUVIDORIA - Você deve agir estritamente como um formatador de texto jurídico invariável para a Companhia Águas de Joinville.
             ...
             - Parágrafo 02: Deve citar o AI nº ${dadosAnalise.dadosTemplate.numAutoInfracao} gerado em ${dadosAnalise.dadosTemplate.dataGeracaoAI}. 
               ...
               > Se LACRE (Art. 144, XV): "Data da constatação: ${dadosAnalise.dadosTemplate.dataConstatacaoInfracao}. Constatação via leitura. Caso fosse executado a padronização da ligação em até 90 dias, não teria sido aplicado multa."
          ...
          dataConstatacao: "", protocolo: numProcesso, funcionario: "Analista", equipe: "Ouvidoria"
        }),
      });

      if (!response.ok) throw new Error("Erro ao processar as regras textuais.");
      ...
      console.error(error);
      alert("Falha ao redigir parecer técnico com a base padrão. Verifique o servidor.");
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="h-full flex flex-col">
      {/* Top Bar - FORMATADO IGUAL À NOTIFICAÇÃO */}
      ...
      <div className="flex-1 overflow-auto bg-[#f8fafe]">
        <div className="p-8 max-w-5xl mx-auto space-y-6">
          ...
          {/* SESSÃO 2: TRIAGEM DO HISTÓRICO E INFRAÇÃO */}
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-visible">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
              ...
              <div>
                <h2 className="text-[#0b1e35] font-semibold text-sm">Tipo de Infração (Objeto) e Histórico</h2>
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-8">
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-2">Houve defesa prévia?</label>
                  <div className="flex gap-2">
                    <button
                      type="button"
                      onClick={() => setHistoricoDefesa("com_defesa")}
                      className={`flex-1 py-3 px-2 rounded-xl border-2 text-center text-xs font-semibold transition-all ${historicoDefesa === "com_defesa" ? "border-[#1a5fa8] bg-[#eef6ff] text-[#1a5fa8]" : "border-gray-200 bg-white text-gray-500"}`}
                    >
                      Sim (Com Defesa)
                    </button>
                    <button
                      ...
                      className={`flex-1 py-3 px-2 rounded-xl border-2 text-center text-xs font-semibold transition-all ${historicoDefesa === "sem_defesa" ? "border-[#1a5fa8] bg-[#eef6ff] text-[#1a5fa8]" : "border-gray-200 bg-white text-gray-500"}`}
                    >
                      Não (Sem Defesa)
                    </button>
                  </div>
          ...
          {/* SESSÃO 3: REQUISITOS VARIÁVEIS DO HISTÓRICO */}
          <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-visible">
            <div className="px-6 py-4 border-b border-gray-100 flex items-center gap-3">
            ...
            <div className="p-6">
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                {["servico", "lacre", "corte_cavalete", "corte_ramal", "hd", "bypass", "clandestina"].includes(tipoCaso) && (
                  <>
                    ...
                    {tipoCaso !== "lacre" && (
                      <div>
                        <label className="block text-[10px] font-bold text-amber-600 uppercase tracking-wider mb-1">Nº Prot. Origem/Fiscaliz.</label>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mt-4">
                ...
                <div>
                  <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">Data de Recebimento do AR</label>
              ...
              {/* CAMPOS ADICIONAIS DINÂMICOS PARA CASOS DE CORTE */}
              {["corte_cavalete", "corte_ramal"].includes(tipoCaso) && decisao === "indeferir" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 p-3 bg-amber-50/40 border border-amber-200 rounded-xl animate-fadeIn">
                  <div>
              ...
              {/* Renderiza campos de defesa se aplicável */}
              {historicoDefesa === "com_defesa" && (
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 mt-4 pt-4 border-t border-dashed border-gray-100 animate-fadeIn">
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">Data Protocolo Defesa</label>
                    <input value={dataDefesa} onChange={(e) => setDataDefesa(maskDate(e.target.value))} placeholder="DD/MM/AAAA" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1a5fa8]" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">Nº Prot. Defesa</label>
                    <input value={protDefesa} onChange={(e) => setProtDefesa(e.target.value)} placeholder="Ex: Prot. Defesa" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1a5fa8]" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">Data Indeferimento</label>
                    <input value={dataIndeferimento} onChange={(e) => setDataIndeferimento(maskDate(e.target.value))} placeholder="DD/MM/AAAA" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1a5fa8]" />
                  </div>
                  <div>
                    <label className="block text-[10px] font-semibold text-gray-500 uppercase tracking-wider mb-1">Nº Prot. Indeferimento</label>
                    <input value={protIndeferimento} onChange={(e) => setProtIndeferimento(e.target.value)} placeholder="Ex: Prot. Indeferimento" className="w-full px-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:border-[#1a5fa8]" />
                  </div>
                </div>
              )}
          ...
          >
            {loading ? (
              <>
                ...
                Injetando Variáveis no Template Oficial...
              </>
            ) : (
              <>
                <Sparkles size={18} />
